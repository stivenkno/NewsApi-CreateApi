...
router.get('/health',async  (req, res) => {
    try {
        const response = await axios.get('https://eventregistry.org/api/v1');

        if (response.status >= 200 && response.status < 300) {
            res.json({
                message: 'Conexión a la API verificada correctamente',
                status: response.status,
                
            });
        } else {
            res.status(response.status).json({
                error: `Error en la solicitud a la API: ${response.statusText}`
            });
        }
